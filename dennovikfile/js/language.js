(function () {
  ...
  const callback = (lan, newLan, oldLan) => {
    return (e) => {
  ...
  if (active === 'toru') {
    callback(ru, toRu, toEng)();
  }
  toEng.addEventListener('click', callback(eng, toEng, toRu));
  toRu.addEventListener('click', callback(ru, toRu, toEng));
})();
